"""
console.py — the game-AGNOSTIC console toolkit. Carried VERBATIM from LLD-23's
Tic-Tac-Toe package: it knows nothing about parking, so it transfers untouched.
That portability is the proof it was scoped right — master it once, reuse forever.

One core loop (ask) + the question shapes nearly every CLI round needs: a bounded
number, a non-empty string, a menu, an enum, a yes/no. Zero dependencies.
"""
...
def ask(prompt: str, parse: Callable[[str], T] = str,
    ...
    at the boundary — the service never sees a raw string.
    """
    while True:
