...
A user inserts a card, authenticates with a PIN, and withdraws/deposits/checks balance; the machine is a state machine over hardware.

> **Difficulty:** Medium &nbsp;·&nbsp; **Key concepts:** State machine, card/PIN auth, cash dispensing
...
<details>
  ...
  1. A user authenticates with a card and PIN.
  2. Limited PIN attempts before the card is blocked.
  3. Operations: balance enquiry, withdrawal, deposit, mini-statement.
  4. Withdrawal checks balance and ATM cash availability.
  5. Dispense cash using available denominations.
  6. The machine moves through states: idle → card inserted → authenticated → transaction → eject.
  7. Each transaction is atomic and logged.
  8. Eject the card and reset to idle on completion or timeout.
</details>
