...
Board & Strategy Games LLD problems test **game loop, turn management, move validation**. Work the [playbook](#the-lld-playbook-use-it-on-every-problem) below on each worksheet in this folder, then reveal the sample requirements to check yourself.

## Case studies in this category

| Case study | Difficulty | Key concepts |
|---|---|---|
| Chess | Hard | Piece inheritance, move validation, check/checkmate |
| Snake & Ladder | Easy | Random dice, board traversal, win condition |
| Sudoku | Medium | Grid validation, constraints, hints |
