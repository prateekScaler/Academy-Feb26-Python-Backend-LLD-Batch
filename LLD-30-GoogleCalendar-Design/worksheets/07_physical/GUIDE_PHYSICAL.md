...
Real-world Physical Systems LLD problems test **resource allocation, state management**. Work the [playbook](#the-lld-playbook-use-it-on-every-problem) below on each worksheet in this folder, then reveal the sample requirements to check yourself.

## Case studies in this category

| Case study | Difficulty | Key concepts |
|---|---|---|
| Parking Lot | Medium | Spot allocation, pricing, ticketing |
| Elevator | Medium | Request scheduling, direction, state |
| Vending Machine | Easy | State machine, inventory, change |
