...
Booking & Reservation Systems LLD problems test **seat selection, concurrency, payment flow**. Work the [playbook](#the-lld-playbook-use-it-on-every-problem) below on each worksheet in this folder, then reveal the sample requirements to check yourself.

## Case studies in this category

| Case study | Difficulty | Key concepts |
|---|---|---|
| Movie Ticket Booking | Medium | Seat locking, show management, concurrency |
| Hotel Reservation | Medium | Room types, date-range booking, availability |
| Flight Booking | Hard | Seat classes, passengers, PNR |
