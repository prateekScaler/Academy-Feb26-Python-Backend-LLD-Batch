...
| 10-11 | [Tic-Tac-Toe &mdash; Design &amp; Code](LLD-23-TicTacToe-Code/) | From the decided classes to a running game: re-derive the relationships (composition vs aggregation via the lifetime question), settle the reference class diagram, freeze the API signatures, then build the engine live &mdash; a single teaching file **and** a packaged tree (models / strategies / game / `console.py` toolkit / CLI). Win-check complexity tiers (O(n&sup2;&middot;k) &rarr; O(k) &rarr; O(1)). Companion challenges open as Issues #14&ndash;16 (code smells, undo, winner-check). |
...
&#127919; **[LLD-23 &mdash; Tic-Tac-Toe: Code](LLD-23-TicTacToe-Code/)** &mdash; from the decided classes to a running game. Re-derive every relationship (composition vs aggregation via the lifetime question), settle the reference class diagram, freeze the API signatures, then build the whole engine live &mdash; both as a single teaching file (`code/01_tictactoe.py`) and as a properly packaged tree (`models/` &middot; `strategies/` &middot; `game.py` &middot; a game-agnostic `console.py` input toolkit &middot; `cli.py`). Win-check complexity tiers (O(n&sup2;&middot;k) &rarr; O(k) &rarr; O(1)), Gomoku as a two-argument extension, and a trade-offs close on undo, persistence and Connect-Four. Three companion challenges are open as GitHub Issues for students to raise PRs &mdash; **#14** (code smells), **#15** (undo), **#16** (winner-check, made configurable). Built on the LLD-22 design and Discussion #12 submissions.
