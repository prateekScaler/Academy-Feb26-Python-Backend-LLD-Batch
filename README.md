...
&#127919; **[LLD-30 &mdash; Google Calendar: Design](LLD-30-GoogleCalendar-Design/)** &mdash; the playbook on a **time-shaped** problem. A scenario intro (reveal the design questions one at a time) &rarr; **FR-by-FR class derivation** (each FR first, then a collapsible that derives the class + attributes) &rarr; the hard parts &rarr; the full **class diagram**. The open variable is **recurrence** (store the *rule* and expand on read; a single occurrence changes via **EXDATE + override**); the course&rsquo;s recurring shape is the **Attendee** association class (event &times; user, RSVP on the join); the time twists are **time zones** (store UTC, render local, with DST spring-forward / fall-back edge cases) and **free/busy** (interval overlap + merge-the-gaps to find a common slot). Three companions ship in the same interactive style: a **database-design** walk-through (the same problem via **Minimal Modeling** &mdash; anchors &rarr; attributes &rarr; links &rarr; SQL, with an ER diagram), a **&ldquo;Falsehoods programmers believe about time&rdquo;** true/false quiz (each statement with the gotcha, the fix, and annotated Python), and the quick-revise **interview-prep handbook** (10 design patterns with generic UML) plus the 20-problem **worksheet library**. Design today; the class diagram + REST API are homework &mdash; [Discussion #29](https://github.com/prateekScaler/Academy-Feb26-Python-Backend-LLD-Batch/discussions/29).
