...
&#129514; **[LLD-31 &mdash; Unit Testing: Why, pytest &amp; Best Practices](LLD-31-Testing-1-Unit-Testing-Best-Practices/)** &mdash; **Module 4 (Advanced Software Engineering) kicks off.** Tests aren&rsquo;t about proving code works today &mdash; they **keep it working tomorrow**. Opens with the bug nobody notices (Knight Capital&rsquo;s $440M / 45&nbsp;min, then a coupon-ordering refactor that silently overcharges &#8377;5), then the **testing pyramid** (unit / integration / E2E, each a click-to-reveal row naming its tooling &amp; who owns it), **pytest** fundamentals (discovery by naming, bare-<code>assert</code> rewriting, the daily flags, unittest vs pytest), and the craft: **AAA**, behaviour-naming and **FIRST** taught as discover-it-yourself quizzes, **parametrize** over boundary tables, **fixtures** as dependency injection, testing **errors** (<code>pytest.raises</code>) and the **float trap** (money = integer paise), and testing **time** by injecting a <code>Clock</code>. Ends with an **evals teaser** &mdash; when a backend calls an LLM there&rsquo;s no single right answer, so the <code>assert</code> becomes a graded **pass rate** (the deep-dive rides with next class&rsquo;s mocking). **[`code/`](LLD-31-Testing-1-Unit-Testing-Best-Practices/code/)** ships 8 files &mdash; 7 worked pytest suites (44 tests, green) plus **[`08_homework.py`](LLD-31-Testing-1-Unit-Testing-Best-Practices/code/08_homework.py)**, a cinema backend with 12 `pytest.skip` stubs for students to turn green.
