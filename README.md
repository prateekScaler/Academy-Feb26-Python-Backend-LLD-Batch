...
| 15 | Code BookMyShow | Upcoming |
| 16-17 | Design & Code Splitwise | Upcoming |
| 18 | Design Google Calendar | Upcoming |

## Latest

&#127919; **[LLD-26 &mdash; BookMyShow: Design](LLD-26-BookMyShow-Design/)** &mdash; the concurrency case study, same playbook. Open with a one-line overview, clarify the scope, then split the FRs into **structural** (the nouns &rarr; the class diagram) vs **behavioural** (the verbs &rarr; the APIs). A use-case diagram turns each actor into an endpoint; the seat-map and user-journey pictures hold the whole problem. Entities are **derived round-by-round** &mdash; the obvious nouns first, then the subtle one you have to *think* your way to: the **ShowSeat** (the seat &times; show pairing carrying per-show status + price). Settle the "class or table?" question (an **association class is an associative table** &mdash; same entity, two layers), derive every FK through quizzes, and read an **ER diagram off the class diagram**. Concurrency is previewed as a menu &mdash; code lock / pessimistic / optimistic / DB `UNIQUE` / Redis, and the hold (`HOLD &rarr; pay &rarr; CONFIRM`) &mdash; with "which is best?" deferred to LLD-27. Homework: class diagram ([Discussion #23](https://github.com/prateekScaler/Academy-Feb26-Python-Backend-LLD-Batch/discussions/23)) + REST API ([Discussion #22](https://github.com/prateekScaler/Academy-Feb26-Python-Backend-LLD-Batch/discussions/22)), with a [Class Diagram Tools Guide](LLD-26-BookMyShow-Design/CLASS-DIAGRAM-TOOLS-GUIDE.md).
