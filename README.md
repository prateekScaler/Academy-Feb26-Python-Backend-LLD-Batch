...
&#127919; **[LLD-29 &mdash; Splitwise: Code](LLD-29-Splitwise-Code/)** &mdash; the LLD-28 design, **built**. A single-file engine and a layered package (`models / strategies / balance_sheet / debt_simplifier / service`), both assert green. We derive the classes **FR-by-FR** &mdash; the `Split` / **UserExpense** association (how who-paid and who-owes normalise into one junction table), the **Membership** join and **RBAC** role &mdash; then write the engine: the **SplitStrategy** family with the **penny fix** (integer **paise** + a `divmod` remainder, so ₹100/3 sums exactly), the self-netting **BalanceSheet** (opposite debts cancel; an O(1) "what do I owe?"), and the headline **debt simplification**: the problem via graphs, three approaches &mdash; **greedy net-and-match** (= LeetCode 465, the recommended heuristic), **cycle-cancelling**, and the **optimal subset-sum** &mdash; with a plain-English **"what is NP-hard?"** explainer (Sudoku: easy to check, hard to solve) and a greedy iteration walkthrough. Plus **normalization** (1NF/2NF/3NF MCQs + a concrete 3NF split), **indexes** (composite + the leftmost-prefix rule), and the object&harr;schema bridge. Built on the LLD-28 design; discussions [#26](https://github.com/prateekScaler/Academy-Feb26-Python-Backend-LLD-Batch/discussions/26)/[#27](https://github.com/prateekScaler/Academy-Feb26-Python-Backend-LLD-Batch/discussions/27)/[#28](https://github.com/prateekScaler/Academy-Feb26-Python-Backend-LLD-Batch/discussions/28).
